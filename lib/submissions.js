...
/**
 * Resolve department details from either name, ID, slug, or legacy alias.
 */
export function resolveDepartment(departmentIdentifier) {
  if (!departmentIdentifier) return null;
  const clean = String(departmentIdentifier).trim().toLowerCase();
  const match = reviews.find(
    (dept) =>
      dept.id.toLowerCase() === clean ||
      dept.name.trim().toLowerCase() === clean ||
      (dept.slug && dept.slug.toLowerCase() === clean) ||
      (dept.aliases && dept.aliases.some((a) => a.toLowerCase() === clean))
  );
  return match || null;
}
...
export function resolveQuestion(deptIdOrName, questionKey) {
  ...
  // Legacy Obfuscated Question Aliases (For 100% historical test & DB compatibility)
  const legacyMap = {
    "poAx ZQPF iL0C *$ Peq# 2qu43N8V0TC8 c3^ pKl1ypL *D@DRew tB Dm2 pwS#kXdV5XM?": { id: "q_app_01", type: "generic" },
    "2#Y 73p₹xw kn qhP 77xDQWeB9 g*h?": { id: "q_web_05", type: "short-text" },
    "yi8gnK7 VJW9 ^Q9Oqo FsC7avP 0PA": { id: "q_web_01", type: "long-text" },
    "Why do you want to join Organization Name?": { id: "q_general_why_join", type: "long-text" },
    "Why do you want to join DWASFW?": { id: "q_general_why_join", type: "long-text" },
  };

  if (legacyMap[keyStr]) {
    return {
      id: legacyMap[keyStr].id,
      questionId: legacyMap[keyStr].id,
      name: keyStr,
      questionText: keyStr,
      type: legacyMap[keyStr].type,
      version: 1,
    };
  }
...
export function normalizeSubmission(rawDoc) {
  ...
  if (schemaVersion >= 2 && rawData.submissionId && Array.isArray(rawData.answers)) {
    ...
    return {
      ...
      currentPhase: rawData.currentPhase ?? (rawData.shortlisted ? 4 : (rawData.phase ?? 1)),
      phaseName: rawData.phaseName || (rawData.currentPhase === 6 ? "Selected" : rawData.currentPhase >= 4 || rawData.shortlisted ? "Interview" : rawData.currentPhase === 3 ? "Domain Review" : rawData.currentPhase === 2 ? "Screening" : "Applied"),
      scores: rawData.scores || {},
      rubric: rawData.rubric || {},
      interviewSlot: rawData.interviewSlot || null,
